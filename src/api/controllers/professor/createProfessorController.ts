...
class CreateProfessorController implements BaseController {
    ...
    async Handle(req: Request, res: Response): Promise<Response>{
        try {
            const { name, email, registration, telephone, coordinationId, password } = req.body || {}
            const dto = new CreateProfessorInputDTO(name, email, registration, telephone, coordinationId, password)
