...
class ProfessorServices implements IProfessorServices {
    ...
    async CreateAsync(data: ProfessorDTO) {
        InfrastructureException.When((await this.professorRepository.FindByEmail(data.email) as Professor | null) !== null, InfrastructureExceptionNameEnum.CONSTRAINT_ERROR, "Email already in use", 409)
    ...
    async UpdateAsync(id: number, data: ProfessorDTO) {
        if (!(await this.GetByIdAsync(id) as Professor))
            throw new ApplicationException(ApplicationExceptionNameEnum.NOT_FOUND, "No professor was found with the provided id", 404)
